...
export function StandingIn() {
  ...
  return (
    <section className="relative isolate mx-auto flex max-w-7xl flex-col justify-center px-6 py-[clamp(7rem,16vh,11rem)] sm:px-10 lg:min-h-[100svh]">
      <Scrub>
      ...
      <Scrub>
        <p className="mt-4 max-w-2xl text-muted">
          {ORDER.length} themes, one vocabulary. switch it in the nav. nothing
          forks.
        </p>
      </Scrub>
      ...
      <Drift className="mt-12">
        <AnimatePresence mode="wait">
          <motion.article
            ...
            exit={
              reduced ? undefined : { opacity: 0, filter: "blur(10px)", y: -6 }
            }
            transition={{ duration: 0.55, ease: COOL }}
            className="relative overflow-hidden rounded-2xl border border-border bg-surface/30 shadow-floating"
          >
            <div
              aria-hidden="true"
              ...
              style={{
                backgroundImage:
                  "radial-gradient(90% 70% at 8% 0%, color-mix(in oklab, var(--usva-accent) 22%, transparent), transparent 65%), radial-gradient(70% 60% at 95% 100%, color-mix(in oklab, var(--usva-accent-alt) 14%, transparent), transparent 70%)",
              }}
            />
            <div className="grid min-w-0 gap-10 p-8 sm:p-12 lg:grid-cols-[1.05fr_1fr] lg:gap-20 lg:p-14">
              <div className="flex flex-col gap-4">
                <p className="font-mono text-xs uppercase tracking-[0.25em] text-muted">
                  theme {note.index} of {ORDER.length}
                </p>
                <h3 className="text-5xl font-extrabold tracking-[-0.04em] text-ink sm:text-6xl">
                  {note.word}
                </h3>
