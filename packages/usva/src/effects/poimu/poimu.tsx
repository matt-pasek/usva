...
import {
  useThemeVersion,
  useTokenColors,
} from "../effects-core/use-token-colors.js";
import {
  approach,
  keyLight,
  POINTER_EASE,
...
import { poimuFragmentShader } from "./poimu-shader.js";
import {
  poimuUniforms,
  setPoimuColors,
  setPoimuFrame,
  setPoimuParams,
} from "./poimu-uniforms.js";
...
export interface PoimuProps extends React.HTMLAttributes<HTMLDivElement> {
  ...
  params?: Partial<PoimuParams>;
  children?: React.ReactNode;
}

export const Poimu = React.forwardRef<HTMLDivElement, PoimuProps>(
  (
    {
      speed = 1,
      interactive = true,
    ...
    const paramsRef = React.useRef<PoimuParams>(resolveParams(params));
    ...
    const lamps = React.useMemo<PoimuColors>(() => {
    ...
    const canvas = useGlCanvas({
      fragment: poimuFragmentShader,
      uniforms: () => poimuUniforms(lampsRef.current, paramsRef.current),
      pointer: true,
      ...
      onFrame: (u, frame) => {
        ...
        setPoimuColors(u, lampsRef.current);
        setPoimuParams(u, paramsRef.current);
        setPoimuFrame(u, {
