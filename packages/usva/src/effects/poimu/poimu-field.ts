...
/**
 * poimu is a pleat: a vast sheet of silk, creased a few times and lit from three
 * sides. Nothing here samples a palette. The colour is what the lighting does to
 * the surface, so a fold turning toward one lamp reads as that lamp's hue and the
 * same fold rolling away catches the next. The ground is black because no light
 * reaches it, not because it was painted black.
 */
export interface PoimuParams {
...
export const POIMU_DEFAULTS: PoimuParams = {
  scale: 1.2,
  relief: 3.0,
  crease: 1.15,
  creaseWidth: 1.0,
...
export function resolveParams(overrides?: Partial<PoimuParams>): PoimuParams {
  return { ...POIMU_DEFAULTS, ...overrides };
}
