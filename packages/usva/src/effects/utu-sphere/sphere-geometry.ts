/**
 * Pure math for UtuSphere: the static field parameters, the slow breathing and
 * pointer-lean easing, and the plum -> orchid -> hot-magenta colour ramp. No DOM
 * and no sula imports, so the effect stays genuinely standalone (its motion never
...
export const DEFAULT_PARAMS: SphereParams = {
  radius: 1.65,
  bands: 5,
  swirl: 2.2,
  omega: 0.12,
...
export function resolveParams(overrides?: Partial<SphereParams>): SphereParams {
 ...
 */
export const DAWN: SphereColors = {
  deep: [0.2, 0.13, 0.42],
  mid: [0.8, 0.3, 0.72],
  hot: [1.0, 0.72, 0.52],
};
...
export function buildRamp(overrides?: Partial<SphereColors>): SphereColors {
  return {
 ...
 */
export function monoRamp(accent: Rgb): SphereColors {
  return {
