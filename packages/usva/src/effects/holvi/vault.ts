...
export const VAULT_DEFAULTS: VaultParams = {
  pitch: 0.42,
  curve: 0.018,
  fold: 7,
  foldScale: 0.045,
...
export function resolveVaultParams(
  overrides: Partial<VaultParams> = {},
): VaultParams {
  return { ...VAULT_DEFAULTS, ...overrides };
}
...
export function setVaultColors(u: Uniforms, colors: VaultColors): void {
  setUniform(u, "uLow", colors.low);
  setUniform(u, "uHigh", colors.high);
  setUniform(u, "uStar", colors.star);
}

export function setVaultParams(u: Uniforms, params: VaultParams): void {
...
export function setVaultFrame(u: Uniforms, frame: VaultFrame): void {
