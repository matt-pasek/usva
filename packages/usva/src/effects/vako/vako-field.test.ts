...
describe("ridge", () => {
  it("runs from a floor at 0 to a crest at 1", () => {
    expect(ridge(0, VAKO_DEFAULTS.ridgeShape)).toBeCloseTo(0);
    expect(ridge(0.5, VAKO_DEFAULTS.ridgeShape)).toBeCloseTo(1);
    expect(ridge(1, VAKO_DEFAULTS.ridgeShape)).toBeCloseTo(0);
  });
  ...
  it("keeps the floor flatter than the crest, because clay compresses", () => {
    const shape = VAKO_DEFAULTS.ridgeShape;
...
describe("resolveParams", () => {
  it("floors the warp at several furrow spacings, so the rings cannot read as rings", () => {
    ...
    expect(VAKO_DEFAULTS.warpAmt).toBeGreaterThanOrEqual(MIN_WARP_TURNS);
  });
