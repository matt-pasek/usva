  ...
  it("is a unit vector across the whole pointer range", () => {
    for (const x of [-1.5, -0.5, 0, 0.5, 1.5]) {
      for (const y of [-1.5, 0, 1.5]) {
        expect(length(keyLight(KUULTO_DEFAULTS, [x, y], 1))).toBeCloseTo(1, 6);
      }
    }
  });

  it("rests at the configured direction when the pointer is away", () => {
    const at = keyLight(KUULTO_DEFAULTS, [1, 1], 0);
    const rest = keyLight(KUULTO_DEFAULTS, [0, 0], 1);
    expect(at).toEqual(rest);
  });

  it("swings toward the pointer", () => {
    const left = keyLight(KUULTO_DEFAULTS, [-1, 0], 1);
    const right = keyLight(KUULTO_DEFAULTS, [1, 0], 1);
