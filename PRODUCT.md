...
**The installer.** Adds `@usva-ui/react` and `@usva-ui/tokens` as dependencies and lets `bun update` carry fixes forward. Wants a stable prop API, honest TypeScript types, and semantic role tokens they can retheme without forking. Never reads the component source. Breaking a prop name costs them real time.
